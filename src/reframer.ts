...
export const analyseFrames: Valve<beamcoder.Frame[], AnalysisResult> = async (frames) => {
	if (isValue(frames)) {
		...
		const t = startTrace('Detect scenechange')
		const filt_frames = await (await detectFilter).filter(frames)
		endTrace(t)
		...
		if (filt_frames[0].frames[1]) {
			...
			if (frameNo % 12 === 0) {
				try {
					...
					const t = startTrace('Frame Analysis')
...
const getMaxAccel = (target: number, accelObj: { x?: number; dx?: number }): { x: number; dx?: number } => {
...
const cropFrameFromAnalysis = async (
	...
	lastAccelX = getMaxAccel(x, lastAccelX)
